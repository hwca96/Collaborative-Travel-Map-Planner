...
with open("data/vancouver_attractions.json", encoding="utf8") as f:
    ...
    for poi in data['poi']:
        if "Description" in poi:
            cur.execute(f"INSERT INTO Attraction (name, type, lat, lon, address, description) VALUES (?, ?, ?, ?, ?, ?)",
                        (poi['Poiname'], poi['customType'], poi['wLat'], poi['wLon'], poi['Address'], poi['Description'])
                )
        else:
            cur.execute(f"INSERT INTO Attraction (name, type, lat, lon, address) VALUES (?, ?, ?, ?, ?)",
                        (poi['Poiname'], poi['customType'], poi['wLat'], poi['wLon'], poi['Address'])
                )

# Trip and Trip User Records
now = datetime.now()
connection.execute("INSERT INTO Trip (trip_name, trip_created_date) VALUES (?, ?)",  ("test trip", now))
...
# Add comments
connection.execute("INSERT INTO CommentRecord (user_id, trip_attraction_id, comment_text) VALUES (?, ?, ?)",
                     (1, 1, "Must go to this place"))

# Add voting
connection.execute("INSERT INTO VotingRecord (user_id, trip_attraction_id, star_value) VALUES (?, ?, ?)",
                     (1, 1, 10))
